...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH,HEIGHT,"MarchingCube",nullptr,nullptr);
	...
	cout<<"CreateMesh::START::"<<glfwGetTime()<<endl;
...
//	Mesh.createMeshByFunction(glm::vec3(0.0f), glm::vec3(bianshu), bianchang, function1);
	...
	myMesh.createMeshByFunction(glm::vec3(0.0f), glm::vec3(10.0f), 0.7f, function);
	cout << "CreateMesh::END::" << glfwGetTime() << endl;

	cout << "UPDATEMesh::START::" << glfwGetTime() << endl;
	myMesh.update();
	cout << "UPDATEMesh::END::" << glfwGetTime() << endl;

	Shader Mshader("Shaders/VertexShader.glsl", "Shaders/FragmentShader.glsl");
	Shader Lshader("Shaders/lineVShader.glsl", "Shaders/lineFShader.glsl");
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (mouses[GLFW_MOUSE_BUTTON_RIGHT])
	{
		...
		rotateD = sqrt(xoffset*xoffset + yoffset*yoffset)*0.1f;
